...
<div class="pd-20">
    <div class="row cl text-r">
        <?php if(!empty($files)): ?><span class="col-1 pr-10">文件列表：</span>
            <table class="table table-border table-bordered table-hover table-bg table-sort col-11">
                <thead>
                <tr>
                    <th width="6%">序号</th>
                ...
                <?php if(is_array($files)): $i = 0; $__LIST__ = $files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$file): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo date('Y-m-d',$file['uploadtime']);?></td>
                        <td><?php echo ($file['worker']); ?></td>
                        <td><a href="/monitor/uploads/<?php echo ($file['file']); ?>">下载</a> <a title="删除" href="javascript:;" onclick="file_del(this,<?php echo ($file['id']); ?>)" class="ml-5" style="text-decoration:none" id="<?php echo U('Pmitem/fileDelete',array('id'=>$file['id']),'');?>">删除</a></td>
                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                </tbody>
            </table><?php endif; ?>
    </div>
    <input type="hidden" id="imgPath" value="/monitor/Public/admin/images" />
    <form action="" method="post" class="form form-horizontal" id="form-article-add">
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>承办人：</label>
            <div class="formControls col-10">
                <input type="text" class="input-text" placeholder="" name="worker">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1">履责情况：</label>
            <div class="formControls col-11">
                <div class="uploader-list-container">
                    <div class="queueList">
                        <div id="dndArea" class="placeholder">
                            <div id="filePicker-2"></div>
                            <p>或将文件拖到这里，单次最多5个文件</p>
                        </div>
                    </div>
                    <div class="statusBar" style="display:none;">
                        ...
                        <div class="btns">
                            ...
                            <input type="hidden" value="<?php echo U('Pmitem/uploadFiles','','');?>" id="uploadUrl"/>
                            <input type="hidden" value="<?php echo ($inId); ?>" id="pid"/>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        var uploader = WebUploader.create({
            ...
            accept: {
                title: 'Images',
                extensions: 'txt,pdf,doc,docx,gif,jpg,jpeg,bmp,png',
                mimeTypes: 'text/plain,application/pdf,application/msword,image/*'
            }
        });
        uploader.on( 'fileQueued', function( file ) {
            ...
            uploader.makeThumb( file, function( error, src ) {
                if ( error ) {
                    $img.replaceWith('<span>不能预览1</span>');
                    return;
                }
                var src =$("#imgPath").val();
                $img.attr( 'src', wordsrc);
                //               $img.attr( 'src', src );
            ...
            function addFile( file ) {
                ...
                if ( file.getStatus() === 'invalid' ) {
                    ...
                    uploader.makeThumb( file, function( error, src ) {
                        uploadFile = file;
                        var img;
                        //   src = $("#imgPath").val()+'/word.jpg';
                        var src;
                        if ( error ) {
                            if(file.ext == "doc" || file.ext == "docx"){
                                src = $("#imgPath").val()+'/word.jpg';
                            }else if(file.ext == "pdf"){
                                src = $("#imgPath").val()+'/pdf.jpg';
                            }else if(file.ext == "txt"){
                                src = $("#imgPath").val()+'/txt.jpg';
                            }else if(file.ext == "zip" || file.ext == "rar"){
                                src = $("#imgPath").val()+'/zip.jpg';
                            }else{
                                src = $("#imgPath").val()+'/wenhao.jpg';
                            }
                        }
    ...
    $(function(){
        ...
        $("#editor").html("（1）坚持党管干部原则和正确用人导向，严格按照“对党忠诚、善谋打仗、敢于担当、实绩突出、清正廉洁”的军队好干部标准选拔任用，严格执行干部人事政策，健全完善选人用人机制，严格按制度和程序办事，培养选拔部队工作需要、官兵公认的好干部。<br/>（2）建立领导干部选拔任用纪实制度，对领导干部选拔任用工作的全过程（初始提名、资格审查、民主推荐、研究确定拟考察人选、组织考察、民主测评、公示情况等）进行如实记录，为开展倒查追责提供依据；落实干部提拔使用提交党委会讨论决定前征求支队纪委意见制度。<br/>（3）建立完善于市纪委、检察院反渎职部门的沟通、协调机制，切实履行好职责。（4）落实经济责任审计制度和执法责任审计制度。<br/>（5）落实副团职以下干部能上能下制度，从严管理干部，推动形成能者上、庸者下、劣者汰的用人导向。<br/>（6）建立完善营、连职干部非领导职务晋升和干部轮岗交流制度，激发队伍活力，调动干部积极性。<br/>（7）组织对拟提拔任用干部个人有关事项报告情况进行核实；根据需要组织对干部个人有关事项报告进行抽查核实。（8）完善团职干部、营职大队主官、机关科长和部门副职廉政档案。<br/>（9）构建选人用人监督机制和责任追究机制，加强对营职以下干部选拔任用工作的全程监督，严肃查处违反组织人事纪律行为、追究用人失察失误责任，坚决纠正和防止选人用人不正之风和腐败问题。")
    });
    /*事项-删除*/
