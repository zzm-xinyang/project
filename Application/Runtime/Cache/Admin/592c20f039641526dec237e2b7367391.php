...
<div class="pd-20">
    <form action="<?php echo U('update','','');?>" method="post" class="form form-horizontal" id="form-article-add" >
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>监控重点：</label>
            <div class="formControls col-11W duoxuan">
                <ul class="list">
                   <?php if(is_array($flows)): $i = 0; $__LIST__ = $flows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$flow): $mod = ($i % 2 );++$i;?><li><input type="checkbox" name="pid[]" <?php if($flow['choise'] == 1): ?>checked<?php endif; ?> value="<?php echo ($flow['id']); ?>"><?php echo ($flow['power']); ?></li>&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            </div>
        </div>
