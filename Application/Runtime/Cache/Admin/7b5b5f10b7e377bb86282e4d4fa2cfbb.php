...
<div class="pd-20">
	...
	<form action="<?php echo U('Reports/reportSearch','','');?>" method="post" >
	<div class="text-r"> 
		<label>状态：</label>
...
<table class="table table-border table-bordered table-hover table-bg table-sort">
	...
	<tbody>
		<?php if(is_array($reportLists)): $i = 0; $__LIST__ = $reportLists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$report): $mod = ($i % 2 );++$i;?><tr class="text-c">			
			<td  class="text-c"><input type="checkbox" value="<?php echo ($report['id']); ?>" name="ids"></td>
			<td><?php echo ($i); ?></td>
			<td class="text-l"><?php echo ($report['rname']); ?></td>
			<td><?php if($report['unit']==1): ?>司令部
    			<?php elseif($report['unit']==2): ?> 政治处
    			<?php elseif($report['unit']==3): ?> 后勤处
    			<?php elseif($report['unit']==4): ?> 防火处<?php endif; ?></td>
			<td><?php echo ($report['dname']); ?></td>
			<td><?php echo ($report['title']); ?></td>
			<td><?php echo ($report['report']); ?></td>
			<td><?php if($report['status']==1): ?>待处理
    			<?php else: ?> 已处理<?php endif; ?></td>
			<td><a title="详情" class="ml-5" onclick="member_edit('详情','<?php echo U('Reports/reportFind',array('id'=>$report['id']),'');?>','4','','510')" href="javascript:;"><i class="Hui-iconfont">&#xe695;</i></a>
				 <?php if($report['status']==1): ?><a title="处理" onclick="solve(this,<?php echo ($report['id']); ?>)" href="javascript:;"><i class="Hui-iconfont">&#xe603;</i></a><?php endif; ?>
			<a title="删除" href="javascript:;" onclick="member_del(this,<?php echo ($report['id']); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.post('<?php echo U('Reports/reportDel','','');?>',{'id':id},function(result){
    		if(result==true){    			
...
function solve(o,id){	
	$.post('<?php echo U('Reports/reportSolve','','');?>',{'id':id},function(result){
    		if(result==true){    			
...
function datadel(){
	layer.confirm('确认要删除所有选中的吗？',function(index){
		...
		$.post('<?php echo U('Reports/reportsDelAll','','');?>',{'ids':ids},function(result){
    		if(result==true){    			
				layer.msg('已删除!',{icon:1,time:1000});
				location.reload();				
    		}else{    			
