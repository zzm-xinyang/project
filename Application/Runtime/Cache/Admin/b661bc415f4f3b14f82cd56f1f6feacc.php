...
<body>
<div class="pd-20">
    <form action='<?php echo U("Inlist/itemUpdate","oid=$oid","");?>' method="post" class="form form-horizontal" id="form-article-add">
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>落实事项：</label>
            <div class="formControls col-10">
                <input type="text" class="input-text" id="" name="item" value="<?php echo ($item['item']); ?>">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>责任主体：</label>
            <div class="formControls col-10">
                <input type="text" class="input-text" placeholder="" name="subject" value="<?php echo ($item['subject']); ?>">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1">事项描述：</label>
            <div class="formControls col-10">
                <textarea name="description" cols="" rows="" class="textarea"><?php echo ($item['description']); ?></textarea>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>完成时限：</label>
            <div class="formControls col-10">
                <input type="text" class="input-text" value="<?php echo ($item['timelimit']); ?>" placeholder="" name="timelimit">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>完成情况：</label>
            <div class="formControls col-11">
                <label><input type="radio" value="1" name="status" <?php if(($item['status'] == 1)): ?>checked<?php endif; ?>>待完成</label>&nbsp;&nbsp;
                <label><input type="radio" value="2" name="status"  <?php if(($item['status'] == 2)): ?>checked<?php endif; ?>>已完成</label>&nbsp;&nbsp;
                <label><input type="radio" value="3" name="status"  <?php if(($item['status'] == 3)): ?>checked<?php endif; ?>>未完成</label>
            </div>
        </div>
