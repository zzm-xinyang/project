...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 监督责任 <span class="c-gray en">&gt;</span> 纪委领导班子集体责任 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
    <div class="pd-15 radius lh-22 mb-5" style="background: #eee;">
        <h4><strong>纪委领导班子集体责任清单</strong>&nbsp;<a class="btn btn-primary radius pl-10 pr-10 add" onclick=jtzr_add('添加集体责任','<?php echo U("Inlist/plists","oid=$oid","");?>') href="javascript:;" >+</a></h4>
        <ul class="list lh-24" type="1" id="list">
            <?php if(is_array($inlists)): $k = 0; $__LIST__ = $inlists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$inlist): $mod = ($k % 2 );++$k;?><li><a href="<?php echo U('Inlist/listFind',array('id'=>$inlist['id']),'');?>"><?php echo ($inlist['item']); ?></a>&nbsp;
                    <a title="修改" class="ml-5" onclick=jtzr_edit("修改","<?php echo U('Inlist/listEdit',array('id'=>$inlist['id']),'');?>") href="javascript:;"><img src="/monitor/Public/admin/images/pen.png" width="12"/></a>&nbsp;
                    <a title="删除" href="javascript:;" onclick="member_del(this,'1')" class="ml-5" style="text-decoration:none" id="<?php echo U('Inlist/listDelete',array('id'=>$inlist['id']),'');?>"><img src="/monitor/Public/admin/images/delete.png" width="12"/></a>
                </li>

                <?php if($k == 1): ?><input type="hidden" id="first" value="<?php echo ($inlist['content']); ?>" /><?php endif; endforeach; endif; else: echo "" ;endif; ?>
        </ul>
    ...
    <div class="pd-15 radius lh-22" style="background: #eee;">
        <h4><strong>监督责任分解清单落实情况</strong>&nbsp;<a class="btn btn-primary radius pl-10 pr-10 add" onclick=jtzr_add('添加事项','<?php echo U("Inlist/initems","oid=$oid","");?>') href="javascript:;" >+</a></h4>
    </div>
    <div class="cl pd-5 bg-1 bk-gray">
        <span class="l"><a href="javascript:;" id="dels" class="btn btn-danger radius "><i class="Hui-iconfont"></i> 批量删除</a></span>
        <form action="<?php echo U('Inlist/listsearch');?>" method="post" class="form form-horizontal" id="form-article-add">
        <div class="text-r" style="float:right;">
            <label>责任主体：</label>
            <select name="subject" style="height: 29px;border:1px #ddd solid;">
                <option value="全部">全部</option>
                <option value="纪委班子">纪委班子</option>
                <option value="纪委书记">纪委书记</option>
                <option value="纪委副书记">纪委副书记</option>
    ...
    <table class="table table-border table-bordered table-hover table-bg table-sort">
        ...
        <tbody>
        <form action="<?php echo U('Inlist/deletes','','');?>" method="post">
        <?php if(is_array($itemLists)): $i = 0; $__LIST__ = $itemLists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td><a  onclick=jtzr_add('上传文件','<?php echo U("Inlist/itemUpload",array("inid"=>$item["id"]),"");?>') href="javascript:;" class="upload">上传文件</a></td>
                <td><?php echo ($item['status']); ?></td>
                <td class="va-m"><img src="/monitor/Public/admin/images/<?php echo ($item['tip']); ?>" /></td>
                <td><a href="" title="显示到前台"><img src="images/eye1.ico"  width="12px" height="12px"/></a>&nbsp;&nbsp;<a title="修改" class="ml-5" onclick=jtzr_edit('编辑落实事项','<?php echo U("Inlist/itemEdit",array("id"=>$item["id"]),"");?>') href="javascript:;"><i class="Hui-iconfont">&#xe6df;</i></a>&nbsp;&nbsp;<a title="删除" href="javascript:;" onclick="item_del(this,'1')" class="ml-5" style="text-decoration:none" id="<?php echo U('Inlist/itemDelete',array('id'=>$item['id'],'oid'=>$plist['oid']),'');?>"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
    ...
    <div class="pd-15 radius lh-22 mt-20" style="background: #eee;">
        <h4 id="pieTitle"><strong><?php echo ($curYear); ?>年<span class="who"></span>纪委监督责任落实统计</strong>
            ...
            <input type="hidden" id="countUrl" value="<?php echo U('Inlist/count',array('oid'=>$oid)),'','';?>" />
