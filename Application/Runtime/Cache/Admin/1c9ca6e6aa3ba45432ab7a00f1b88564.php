...
<div class="pd-15">
    <div class="pd-15 radius lh-22 mb-5" style="background: #eee;">
        <h4><strong>主体责任分解清单落实情况</strong>&nbsp;<a class="btn btn-primary radius pl-10 pr-10 add" onclick="jtzr_add('添加成员','<?php echo U("Inmember/add","oid=$oid","");?>')" href="javascript:;" >+</a></h4>
        <ul class="list lh-24">
            <?php if(is_array($members)): $i = 0; $__LIST__ = $members;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><li>
                    <?php if($who == $m['id']): ?><a href="<?php echo U('Inmitem/index',array('who'=>$m['id']),'');?>"  class="cur"><span><?php echo ($m['duty']); ?></span>：<span><?php echo ($m['name']); ?></span><input type="hidden" value="<?php echo ($m['id']); ?>"/></a>&nbsp;&nbsp;
                        <?php else: ?>
                        <a href="<?php echo U('Inmitem/index',array('who'=>$m['id']),'');?>"><span><?php echo ($m['duty']); ?></span>：<span><?php echo ($m['name']); ?></span><input type="hidden" value="<?php echo ($m['id']); ?>"/></a>&nbsp;&nbsp;<?php endif; ?>

                    <a title="修改" class="ml-5" onclick=jtzr_edit("修改","<?php echo U('Inmember/edit',array('id'=>$m['id']),'');?>") href="javascript:;"><img src="/monitor/Public/admin/images/pen.png" width="12"/><a>&nbsp;
                        <a title="删除" href="javascript:;" onclick="member_del(this,'1')" class="ml-5" style="text-decoration:none" id="<?php echo U('Inmember/delete',array('id'=>$m['id']),'');?>"><img src="/monitor/Public/admin/images/delete.png" width="12"/></a>
                </li><?php endforeach; endif; else: echo "" ;endif; ?>

        </ul>
    </div>
    <div class="pd-15 radius lh-22" style="background: #eee;">
        <h4><strong><span class="who"></span>责任分解清单落实情况</strong>&nbsp;<a class="btn btn-primary radius pl-10 pr-10 add" onclick="item_add(this)" href="javascript:;" id="<?php echo U("Inmitem/add","","");?>">+</a></h4>
    </div>
    <div class="cl pd-5 bg-1 bk-gray">
        <span class="l"><a href="javascript:;" id="dels" class="btn btn-danger radius "><i class="Hui-iconfont"></i> 批量删除</a></span>
        <form action="<?php echo U('Inmitem/listsearch');?>" method="post" class="form form-horizontal" id="form-article-add">
    ...
    <div class="pd-15 radius lh-22 mt-20" style="background: #eee;">
        <h4 id="pieTitle"><strong><?php echo ($curYear); ?>年<span class="who"></span>党委主体责任落实统计</strong>
            ...
            <input type="hidden" id="countUrl" value="<?php echo U('Inmitem/count'),'','';?>" />
