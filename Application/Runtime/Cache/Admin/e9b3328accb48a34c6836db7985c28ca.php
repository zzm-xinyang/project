...
<body>
<div class="pd-20">
    <form action="<?php echo U('update','','');?>" method="post" class="form form-horizontal" id="form-user-character-add" id="form-admin-add">

        <div class="row cl">
            <label class="form-label col-1">角色：</label>
            <div class="formControls col-6">
                <input type="text" class="input-text" value="<?php echo ($group["title"]); ?>" id="user-name" name="user-name" datatype="*2-16" nullmsg="角色名称不能为空">
            </div>
            <div class="col-5"> </div>
        </div>
        <div class="row cl">
                            ...
                            <?php if(is_array($Submodule[$k1-1])): $k2 = 0; $__LIST__ = $Submodule[$k1-1];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($k2 % 2 );++$k2;?><dl class="cl permission-list2">
                                    ...
                                    <dd>
                                        <?php if(is_array($operation[$k1-1][$k2-1])): $k3 = 0; $__LIST__ = $operation[$k1-1][$k2-1];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v3): $mod = ($k3 % 2 );++$k3;?><label class="">
                                                <?php if(in_array($v3['id'],$group['rules'])): ?><input type="checkbox" value=<?php echo ($v3['id']); ?> name="user-Character-0-0-0[]" id="user-Character-0-0-1" checked="checked">
                                                    <?php echo ($v3['title']); ?>
                                                    <?php else: ?>
                                                    <input type="checkbox" value=<?php echo ($v3['id']); ?> name="user-Character-0-0-0[]" id="user-Character-0-0-1">
                                                    <?php echo ($v3['title']); endif; ?>
