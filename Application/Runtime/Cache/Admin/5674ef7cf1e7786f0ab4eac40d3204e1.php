...
<script type="text/javascript">
	...
	function wz_search(){
		...
		var url = "/monitor/index.php/Admin/Zjwz4/zjwzxt4_search";
		$.ajax({
			type: "post",
			url: url,
			dataType: "json",
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 执纪问责 <span class="c-gray en">&gt;</span> 第四种形态管理<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-15">
	<div class="pd-15 radius lh-22 mb-5" style="background: #eee;">
		<h4><strong>第四种形态</strong>&nbsp;<a class="btn btn-primary radius pl-10 pr-10 add" onclick="jtzr_add('添加实施方式','<?php echo U(zjwzxt4_add);?>')" href="javascript:;" >+</a></h4>
		<ul class="list lh-24">
			<?php if(is_array($fmodes)): $i = 0; $__LIST__ = $fmodes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($key == 0): ?><li>
						<a href="#" class="cur" ><?php echo ($vo['mode']); ?></a>&nbsp;&nbsp;
							<a title="修改" class="ml-5" onclick="jtzr_edit('修改','<?php echo U(zjwzxt4_edit,array(kw=>$vo[mode]));?>')" href="javascript:;"><img src="/monitor/Public/admin/images/pen.png" width="12"/></a>
							<a>&nbsp;
	...
	<div class="cl pd-5 bg-1 bk-gray">
		...
		<div class="text-r" style="float:right;">
			<input type="hidden" id="ssfsname" name="ssfsname" value="涉嫌违法立案审查">
			<label>涉及类型：</label>
		...
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			<thead>
				<tr class="text-c">
					...
					<th width="70px">实施主体</th>
					<th colspan="3">涉嫌违法立案审查对象</th>
					<th rowspan="2" width="80px">涉嫌违法立案审查时间</th>
					<th rowspan="2">涉嫌违法立案审查摘要</th>
			...
			<tbody>
				<?php if(is_array($dnjgData)): $i = 0; $__LIST__ = $dnjgData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td class="text-c"><input type="checkbox"value="<?php echo ($vo["id"]); ?>" name="id[]"></td>
					<td class="text-c"><?php echo ($i); ?></td>
					<td><?php echo ($vo['unit1']); ?></td>
					...
					<td><a title="修改" class="ml-5" onclick="jtzr_edit('修改执纪问责','<?php echo U(zjwzxt4_edit_wz,array(rid=>$vo[id]));?>')" href="javascript:;"><i class="Hui-iconfont">&#xe6df;</i></a></a> <a title="删除" href="javascript:;" onclick="wz_del(this,'<?php echo ($vo[id]); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
function wz_edit(title){
	var ssfs = document.getElementById("ssfsname").value;
	var realurl = "<?php echo U('zjwzxt4_add_wz');?>?fs="+ssfs;
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		var url = "/monitor/index.php/Admin/Zjwz4/zjwzxt4_del";
		$.ajax({
			type: "post",
			url: url,
			dataType: "json",
...
function wz_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		var url = "/monitor/index.php/Admin/Zjwz4/zjwzxt4_del_wz";
		$.ajax({
			type: "post",
			url: url,
			dataType: "json",
