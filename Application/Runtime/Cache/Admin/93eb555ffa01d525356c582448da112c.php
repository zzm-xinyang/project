...
<div class="pd-20">
    <form action="<?php echo U('save','','');?>" method="post" class="form form-horizontal" id="form-article-add">
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>权力名称：</label>
            <div class="formControls col-10">
                <select name="name" style="height: 29px;border:1px #ddd solid;">
                    <?php if(is_array($flows)): $i = 0; $__LIST__ = $flows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$flow): $mod = ($i % 2 );++$i;?><option value="<?php echo ($flow['id']); ?>"><?php echo ($flow['power']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>权力类别：</label>
            <div class="formControls col-10">
                <label><input type="radio" value="1" name="type" checked>内部管理</label>&nbsp;&nbsp;
                <label><input type="radio" value="2" name="type">社会</label>&nbsp;&nbsp;

            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>设置依据：</label>
            <div class="formControls col-10">
                <input type="text" class="input-text" name="reasons">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>办理主体：</label>
            <div class="formControls col-10">
                <div class="formControls col-10">
                    <input type="text" class="input-text" name="activity">
                </div>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-1"><span class="c-red">*</span>公开范围：</label>
            <div class="formControls col-11">
                <label><input type="checkbox" value="1" name="notice" checked>内部</label>&nbsp;&nbsp;
                <label><input type="checkbox" value="2" name="notice">社会</label>&nbsp;&nbsp;
            </div>
        </div>
        <div class="row cl">
